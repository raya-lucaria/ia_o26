...
```bash
python3 -m pip install -r tools/requirements-regresion.txt
python3 tools/gen_regresion_experimentos.py
```
...
extremos del intervalo observado. Las dos vistas de cada figura muestran
...
conjunto de puntos se usa para ajustar cada grado entero de 1 a 16 y para
calcular su RMSE. `metrics.json` registra esos errores sobre los datos
observados; el menor corresponde al grado 16 en ambos casos.
...
| `lecturas/filosofia_ia/clase_*/introduccion.md` | Fuente única de la introducción general del cuadernillo de lecturas; la página del curso (`course/2_filosofia_ia/1_aceleracionismo/0_index.md`) lleva una copia literal de su sección «Cómo leer este cuadernillo» | `lecturas.py` (la incrusta en el PDF), `test_lecturas.py` (comprueba que la copia de la página del curso no haya derivado) |
