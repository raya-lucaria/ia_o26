...
| `ilustraciones.json` | Catálogo de prompts para `gpt-image-2`. Ya no es un solo estilo: la clave `ilustraciones` (historia) pide fondo transparente, y `ilustraciones_filosofia` pide fondo horneado al color exacto del skin (ver `FONDO_OBJETIVO` en `test_ilustraciones.py`) | `gen_ilustraciones.py` |
